...
public class P2Turn {
	...
	public static void move(Chessboard cb) {
		...
		while( inCheck == true ) {
			...
			if(cb.fields[currRow][currCol].getPiece().getColor() != CreatePlayers.p2color) {
				System.out.println("There is none of your piece at that spot");
				currSpot = in.nextLine();
				inCheck = true;
			}
		}
		...
		while(inCheck) {
			...
			if(cb.fields[inextRow][inextCol].getPiece() != null)
				if(cb.fields[inextRow][inextCol].getPiece().getColor() == CreatePlayers.p2color)
				{
			...
			if( cb.fields[currRow][currCol].getPiece().getName() == 'P') {
				...
				if(cb.fields[currRow + 1][currCol - 1].getPiece() != null) {
					if(cb.fields[currRow + 1][currCol -1].getPiece().getColor() == CreatePlayers.p1color)
			...
			if(!cb.fields[inextRow][inextCol].toString().equals("xx")) {
